...
class IndexSimulationService:
    async def perform_index_simulation(
        ...
        if current_plan:
            ...
            current_rows = current_plan.rows_examined
            current_time = current_plan.estimated_time_ms
        else:
             # Fallback if current_plan creation failed completely (unexpected)
             improvement = 50.0
             current_rows = 10000
             current_time = 500.0
             current_plan = ExplainPlan(
                access_type="ALL", rows_examined=current_rows, key=None, extra="Using where", estimated_time_ms=current_time
             )

        with_index_access_type = "ref"
        with_index_rows = max(1, current_rows // 100)
        ...
        with_index_plan = ExplainPlan(
            ...
            extra="Using index",
            estimated_time_ms=round(current_time * (100 - improvement) / 100, 2)
        )
