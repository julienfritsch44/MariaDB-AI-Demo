...
export function UnifiedQueryAnalyzerClean({ analysisHistory, setAnalysisHistory, onLoadFromHistory, selectedHistoryItem }: UnifiedQueryAnalyzerCleanProps) {
    ...
    const fetchCostEstimate = async (query: string, isOptimized: boolean = false) => {
        try {
    ...
    const fetchWaitEvents = async () => {
        try {
            ...
            if (data.summary.total_lock_waits > 0) {
                setSectionsExpanded((prev: typeof sectionsExpanded) => ({ ...prev, waitEvents: true }))
            }
        } catch (err) {
                    ...
                    <Card id="card-comparison" className="border-2 border-primary/30 bg-primary/5">
                        ...
                        <CardContent className="space-y-4">
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <div className="p-3 rounded-md bg-emerald-500/10 border border-emerald-500/30">
                                        <div className="text-xs text-muted-foreground">Execution Time</div>
                                        <div className="text-2xl font-bold text-emerald-500">{optimizedSandboxResult.result?.execution_time_ms}ms</div>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div className="p-4 rounded-md bg-emerald-500/10 border border-emerald-500/30">
                                    ...
                                    <div className="grid grid-cols-2 gap-4 mb-3">
                                        ...
                                        <div>
                                            ...
                                            <div className="text-xl font-bold text-emerald-500">${optimizedCostEstimate.monthly_cost.toFixed(2)}/mo</div>
                                        </div>
                                    </div>
                            ...
                            <div className="p-4 rounded-md bg-emerald-500/10 border border-emerald-500/30">
                                ...
                                <div className="mt-3 p-2 rounded-md bg-emerald-500/20 border border-emerald-500/40 text-xs text-emerald-300">
                                    <strong>Next step:</strong> Deploy the optimized query to production with confidence
                                </div>
                            </div>
                        </CardContent>
                        )}
